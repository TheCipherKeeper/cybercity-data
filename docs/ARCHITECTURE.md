...
<!--
  Стек/слои/развёртывание — <methodology-repo>/docs/refs/STACKS.md,
  LAYOUT.md, DEPLOYMENT.md; процедура архитектуры —
  <methodology-repo>/docs/guide/10-architecture.md.
...
Модули — реальные Python-пакеты под `src/cybercity_data/`. Слоистая архитектура
CLI: Controller → Service → UseCase → Domain/Data (IO-адаптеры). Швы
usecases/ports/domain/adapters канона MODULE.md применяются по мере
формализации модулей (`ports/` и `adapters/` для broker-producer — Phase 2 TODO,
см. BACKLOG). Текущая реализация:

| Модуль | Роль | Публикует/Читает (топики) |
|---|---|---|
| `domain` | чистая бизнес-логика: `models.py` (Pydantic v2 схема), `allocator.py` (IP/CIDR), `checker.py` (13 cross-field правил) | — |
| `data` | IO-адаптеры: `loader`, `renderer`, `filesystem`, `git`, `zip` | — |
| `use_cases` | оркестрация: `check`, `build`, `init`, `validate_step` (общий pipeline load/allocate/check) | (TODO) `build` → публикует `city.build.completed` |
| `dto` | result DTO use-case'ов (`build_result`, `check_result`, `counts`, `init_result`) | — |
| `services` | application services (CLI facade): `build`, `check`, `init`, `exceptions` | — |
| `controllers` | CLI + presenters: Typer `app`, `commands`, `dependencies`, `responses`, `handlers/{build,check,init}` | — |
| `scenario` (WIP) | авторинг сценариев | — |
| `vuln` (TODO) | авторинг уязвимостей (манифест + overlay-исходники) | — |
...
([`<methodology-repo>`/docs/refs/VERIFICATION.md](https://github.com/TheCipherKeeper/ai-project-template/blob/main/docs/refs/VERIFICATION.md)
+ `guide/40-verify.md`). Бамп пина — отдельным PR. Формат сообщений (envelope:
...
> adapter в `adapters/`) — Phase 2; честно помечено TODO в `docs/BACKLOG.md` и в
...
- Хаб: [`cybercity/COMPOSITION.md`](https://github.com/TheCipherKeeper/cybercity/blob/main/COMPOSITION.md),
  [`cybercity/CONVENTIONS.md`](https://github.com/TheCipherKeeper/cybercity/blob/main/CONVENTIONS.md),
  [`cybercity/adr/`](https://github.com/TheCipherKeeper/cybercity/blob/main/adr/README.md).
- Методология: [`<methodology-repo>`](https://github.com/TheCipherKeeper/ai-project-template)
  (`docs/INDEX.md`, `docs/refs/`, `docs/guide/`).
